...
INPUT=verification/RelayFeeSkim.standard-input.json
...
args=$(cast abi-encode 'constructor(uint256,address)' "$(jq -r .deployment.feeBps "$HASHES")" "$(jq -r .deployment.feeSink "$HASHES")")
...
current=$(forge verify-contract --show-standard-json-input 0x0000000000000000000000000000000000000001 src/RelayFeeSkim.sol:RelayFeeSkim | jq -S "$projection")
