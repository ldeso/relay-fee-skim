...
contract RelayFeeSkimTest is Test {
    ...
    /// @dev The hostile token sorts first, sets up a second claim source for the real token during its own
    ///      forward step, and reenters for that token; without the lock the real token would be taxed twice.
